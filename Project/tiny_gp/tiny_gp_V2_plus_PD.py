...
Training = np.load("/home/bryan/dissertation/Project/Player_Inputs/Dataset/Combined_Dataset/Dataset_Wdupli2.npy", allow_pickle=True)
        ...
        for i in range(y):
            ...
            if ((marioY > 0) and (marioX < 4820)):
                ...
                if(view):
                    ...
                    index = 0
                    for xstate in Xtrain: # loop trought dataset
                        index += 1
                        if (xstate == output).all(): # if this state matches a state in the dataset
                            if [input] != Ytrain[index - 1]: # check if the agent played the same action
                                print(f"not same action i = {i}")
                                punishment[0] += 1 # if not, add one to punishment
                                #time.sleep(0.5)
                            else:
                                print(f"same action i = {i}")
                                #time.sleep(0.5)
                                break # if it dose match, end for loop
            else:
                if(Ended == False):
                    iEnd = i
                    Ended = True
...
def main():      
    ...
    for gen in range(GENERATIONS):        
        ...
        if max(fitnesses) > best_of_run_f:
            ...
            best_of_run.draw_tree("best_of_run_V2_PD",\
    ...
    best_of_run.draw_tree("best_of_run_V2_PD",s)

...
import os
import numpy as np
sys.path.append(os.path.abspath("/home/bryan/dissertation/Project/Player_Inputs/Scripts/Gathering_Input"))
from rominfo import *
import retro
import time
...
def do(env, x, y, view = False, inputs = [] ,states_array = [], distances = []): 
    ...
    iEnd = 0
    Ended = False
    for i in range(y): # run y times
        ram = getRam(env)
        marioX, marioY, layer1x, layer1y  = getXY(ram) # get the X and Y coordinates
        distances.append(marioX) # add the X coordinate to the distance array
        if ((marioY > 0) and (marioX < FINISH)): # if the agent didn't lose a life or make it to the end goal yet
            obs, rew, done, _info = env.step(x)  # Play action x
            if(view): # if the run is being rendered
                saved_inputs = np.array(x)
                saved_inputs = saved_inputs.astype(int)
                inputs.append(saved_inputs) # save action used for best run file

                ram = getRam(env)
                state, xi, yi = getInputs(ram)
                saved_outputs = np.array(state.flatten())
                saved_outputs = saved_outputs.astype(int)
                states_array.append(saved_outputs) # save state of the game for best run file
                env.render()  # Render the environment
        else: # if the run is not being rendered

            # rest is debuging code
            if(Ended == False):
                iEnd = i
                Ended = True
...
def combine(x, y): 
    arr = np.array([0] * 12) # create empty array
    for i in range(12): 
        if x[i] + y[i] > 1:
            arr[i] = 1
        else:
